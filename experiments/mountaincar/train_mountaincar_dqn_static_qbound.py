"""
MountainCar-v0: DQN Static QBound Comparison (4-Way)
Organization: Sparse/State-Dependent Rewards (Static QBound Only)

Environment: MountainCar-v0 (discrete actions, state-dependent negative rewards)
Reward: -1 per step until goal reached
Max steps: 200

WHY NO DYNAMIC QBOUND:
While MountainCar has -1 reward per step, the reward is STATE-DEPENDENT (depends on
whether the goal is reached or not), not purely TIME-STEP dependent. Dynamic QBound
with increasing bounds assumes rewards are determined by the time step within an
episode, independent of state. Here, the total reward depends on WHEN you reach the
goal (a state-dependent event), making static bounds more appropriate.

Comparison:
1. Baseline DQN - No QBound, no Double-Q
2. Static QBound + DQN - Q ∈ [-86.60, 0]
3. Baseline DDQN - No QBound, with Double-Q
4. Static QBound + DDQN - Q ∈ [-86.60, 0] + Double-Q
"""
...
parser = argparse.ArgumentParser(description='MountainCar DQN with static QBound only')
...
RESULTS_FILE = f"/root/projects/QBound/results/mountaincar/dqn_static_qbound_seed{SEED}_in_progress.json"
...
# QBound parameters for MountainCar (State-Dependent Negative Reward)
# Reward: -1 per step until goal
# Max episode length: 200 steps
# Q_min = -1 * (1 - γ^200) / (1 - γ) = -1 * 86.60 = -86.60
# Q_max = 0 (reach goal immediately, terminal state)
QBOUND_MIN = -86.6020325142
QBOUND_MAX = 0.0
...
def train_agent(env, agent, agent_name, max_episodes=MAX_EPISODES, track_violations=False):
    ...
    for episode in tqdm(range(max_episodes), desc=agent_name):
        ...
        while not done and step < MAX_STEPS:
            ...
            # Store transition (no step-aware for state-dependent rewards)
...
def main():
    print("=" * 80)
    print("MountainCar-v0: DQN Static QBound Comparison (4-Way)")
    ...
    print(f"  Reward Structure: State-dependent negative reward")
    ...
    print("  Note: No dynamic QBound (rewards are state-dependent, not time-step dependent)")
    print("=" * 80)
    ...
    if results is None:
        ...
        results = {
            ...
            'script_name': 'train_mountaincar_dqn_static_qbound.py',
            'experiment_type': 'sparse_state_dependent',
            'reward_structure': 'state_dependent',
            'config': {
                'env': ENV_NAME,
    ...
    print("\n" + "=" * 80)
    print("MOUNTAINCAR RESULTS SUMMARY (Final 100 Episodes)")
    print("=" * 80)
    ...
    for method, label in zip(methods, labels):
        ...
        print(f"{label:<25} {mean:>8.2f} ± {std:<12.2f}   {improvement_str:>10}")

    # ===== Save Final Results =====
    timestamp = results['timestamp']
    final_output_file = f"/root/projects/QBound/results/mountaincar/dqn_static_qbound_seed{SEED}_{timestamp}.json"
    os.makedirs('/root/projects/QBound/results/mountaincar', exist_ok=True)
    ...
    print("\n" + "=" * 80)
    print("MountainCar DQN Static QBound 4-Way Comparison Complete!")
    print("=" * 80)
