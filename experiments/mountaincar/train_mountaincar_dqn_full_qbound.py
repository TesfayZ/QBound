"""
MountainCar-v0 DQN with Static QBound

Organization: Time-step Dependent Rewards
This script tests DQN and Double DQN with static QBound on MountainCar.
...
Q_min calculation: Q_min = -1 * (1 - γ^H) / (1 - γ) for H=200, γ=0.99 ≈ -86.60
...
parser = argparse.ArgumentParser(description='MountainCar DQN with static QBound')
...
RESULTS_FILE = f"/root/projects/QBound/results/mountaincar/dqn_full_qbound_seed{SEED}_in_progress.json"
...
# QBound parameters for MountainCar (Dense Negative Reward)
# Reward: -1 per step until terminal
# Q_max = 0 (best case: reach goal immediately)
# Q_min = -1 * (1 - γ^H) / (1 - γ) for H=200, γ=0.99
QBOUND_MIN = -(1 - GAMMA**MAX_STEPS) / (1 - GAMMA)  # ≈ -86.60
QBOUND_MAX = 0.0
...
def main():
    print("=" * 80)
    print("MountainCar-v0: DQN 4-Way Comparison (Time-Step Dependent)")
    ...
    if results is None:
        ...
        results = {
            ...
            'script_name': 'train_mountaincar_dqn_full_qbound.py',
            'config': {
                'env': ENV_NAME,
    ...
    print("\n" + "=" * 80)
    print("MOUNTAINCAR DQN RESULTS SUMMARY (Final 100 Episodes)")
    print("=" * 80)
    print()
    ...
    final_output_file = f"/root/projects/QBound/results/mountaincar/dqn_full_qbound_seed{SEED}_{timestamp}.json"
    ...
    print("\n" + "=" * 80)
    print("MountainCar DQN 4-Way Comparison Complete!")
    print("=" * 80)
    print()
    print("📊 Key Takeaways:")
    print("  - Tests Static QBound on time-step dependent NEGATIVE rewards")
    print("  - Validates QBound effectiveness on challenging exploration task")
    print("  - Q_min = -86.60, Q_max = 0 (negative dense reward)")
