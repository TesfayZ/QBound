"""
Acrobot-v1 DQN with TRANSFORMED Q-Values (Negative → Positive)
...
TRANSFORMATION:
- Original: Q ∈ [-99.34, 0] (negative reward environment)
- Transformed: Q ∈ [0, 99.34] (shifted to positive range like CartPole)
- Method: Add abs(Q_min) = 99.34 to all Q-values

This makes Acrobot's Q-value distribution similar to CartPole's Q ∈ [0, 99.34],
...
2. Static QBound + DQN - Transformed Q ∈ [0, 99.34]
...
parser = argparse.ArgumentParser(description='Acrobot DQN with Transformed Q-values')
...
RESULTS_FILE = f"/root/projects/QBound/results/acrobot/dqn_transformed_seed{SEED}_in_progress.json"
...
# QBound parameters for Acrobot (ORIGINAL negative bounds)
# Original: Q ∈ [-99.34, 0]
# Transformed: Q ∈ [0, 99.34]
QBOUND_MIN_ORIGINAL = -(1 - GAMMA**MAX_STEPS) / (1 - GAMMA)  # ≈ -99.34
...
def main():
    print("=" * 80)
    print("Acrobot-v1: TRANSFORMED DQN (Negative → Positive Q-Values)")
    ...
    if results is None:
        ...
        results = {
            ...
            'script_name': 'train_acrobot_dqn_transformed.py',
            'config': {
                'env': ENV_NAME,
    ...
    print("\n" + "=" * 80)
    print("ACROBOT TRANSFORMED DQN RESULTS (Final 100 Episodes)")
    print("=" * 80)
    print()
    ...
    final_output_file = f"/root/projects/QBound/results/acrobot/dqn_transformed_seed{SEED}_{timestamp}.json"
    ...
    print("\n" + "=" * 80)
    print("Acrobot Transformed DQN Experiment Complete!")
    print("=" * 80)
    print()
    print("📊 Key Insights:")
    print("  - Original Q bounds: [-99.34, 0]")
    print("  - Transformed Q bounds: [0, 99.34] (similar to CartPole)")
