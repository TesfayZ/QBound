"""
Acrobot-v1: DQN Static QBound Comparison (4-Way)
Organization: Sparse/State-Dependent Rewards (Static QBound Only)

Environment: Acrobot-v1 (discrete actions, state-dependent negative rewards)
Reward: -1 per step until goal reached (swing up to height)
Max steps: 500

WHY NO DYNAMIC QBOUND:
While Acrobot has -1 reward per step, the reward is STATE-DEPENDENT (depends on
whether the swing-up goal is reached or not), not purely TIME-STEP dependent.
Dynamic QBound with increasing bounds assumes rewards are determined by the time
step within an episode, independent of state. Here, the total reward depends on
WHEN you reach the goal (a state-dependent event), making static bounds more appropriate.
...
parser = argparse.ArgumentParser(description='Acrobot DQN with static QBound only')
...
RESULTS_FILE = f"/root/projects/QBound/results/acrobot/dqn_static_qbound_seed{SEED}_in_progress.json"
...
# QBound parameters for Acrobot (State-Dependent Negative Reward)
# Reward: -1 per step until swing up
# Max episode length: 500 steps
# Q_min = -1 * (1 - γ^500) / (1 - γ) = -99.34 (using γ=0.99)
# Q_max = 0 (reach goal immediately)
QBOUND_MIN = -99.34295169575846
QBOUND_MAX = 0.0
...
def main():
    print("=" * 80)
    print("Acrobot-v1: DQN Static QBound Comparison (4-Way)")
    ...
    print(f"  Reward Structure: State-dependent negative reward (swing-up task)")
    ...
    if results is None:
        ...
        results = {
            ...
            'script_name': 'train_acrobot_dqn_static_qbound.py',
    ...
    print("\n" + "=" * 80)
    print("ACROBOT RESULTS SUMMARY (Final 100 Episodes)")
    print("=" * 80)
    ...
    final_output_file = f"/root/projects/QBound/results/acrobot/dqn_static_qbound_seed{SEED}_{timestamp}.json"
    os.makedirs('/root/projects/QBound/results/acrobot', exist_ok=True)
    ...
    print("\n" + "=" * 80)
    print("Acrobot DQN Static QBound 4-Way Comparison Complete!")
    print("=" * 80)
