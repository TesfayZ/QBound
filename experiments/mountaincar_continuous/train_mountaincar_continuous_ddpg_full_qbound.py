"""
MountainCarContinuous-v0: DDPG with Soft QBound - Time-Step Dependent Rewards
Organization: Time-step Dependent Rewards

Tests whether DDPG benefits from Soft QBound on continuous control with
dense time-step dependent negative rewards.

Environment: MountainCarContinuous-v0 (continuous actions, dense negative rewards)
Reward Structure: Dense negative rewards per time step (time-step dependent)
Reward range: roughly -1 to 100 (100 for reaching goal)

Comparison:
1. Baseline DDPG - No QBound
2. Static Soft QBound + DDPG - Fixed bounds via softplus_clip

Note: ONLY uses Soft QBound (softplus_clip) because DDPG requires
...
parser = argparse.ArgumentParser(description='DDPG on MountainCarContinuous-v0 with Soft QBound (Time-step Dependent)')
...
RESULTS_FILE = f"/root/projects/QBound/results/mountaincar_continuous/ddpg_full_qbound_seed{SEED}_in_progress.json"

# Environment parameters
ENV_NAME = "MountainCarContinuous-v0"
MAX_EPISODES = 500
MAX_STEPS = 999  # Environment default max steps
EVAL_EPISODES = 10
...
# QBound parameters for MountainCarContinuous
# Reward: roughly -1 per step until goal, +100 for reaching goal
# Worst case: Q_min = -1 * sum(gamma^k for k in 0..998) = -1 * (1-gamma^999)/(1-gamma)
# With gamma=0.99, H=999: Q_min = -1 * 99.99 = -99.99
# Best case: Q_max = 100 (reach goal immediately)
QBOUND_MIN = -99.99
QBOUND_MAX = 100.0
STEP_REWARD = -1.0  # Average reward per step (for dynamic bounds)
...
def main():
    print("=" * 80)
    print("MountainCarContinuous-v0: DDPG 2-Way Comparison (Time-Step Dependent)")
    ...
    if results is None:
        ...
        results = {
            ...
            'script_name': 'train_mountaincar_continuous_ddpg_full_qbound.py',
            'config': {
                'env': ENV_NAME,
    ...
    print("\n" + "=" * 80)
    print("DDPG RESULTS SUMMARY (Final 100 Episodes)")
    print("=" * 80)
    print()
    ...
    methods = [
        ('Baseline DDPG', 'baseline'),
        ('Static Soft QBound + DDPG', 'static_soft_qbound')
    ]
    ...
    final_output_file = f"/root/projects/QBound/results/mountaincar_continuous/ddpg_full_qbound_seed{SEED}_{timestamp}.json"
    ...
    print("\n" + "=" * 80)
    print("MountainCarContinuous DDPG 2-Way Comparison Complete!")
    print("=" * 80)
    print()
    print("📊 Key Takeaways:")
