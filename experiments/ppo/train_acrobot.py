"""
PPO vs PPO+QBound on Acrobot-v1

Acrobot-v1:
- State: 6D continuous (cos/sin of two joint angles, angular velocities)
- Action: 3 discrete (torque: -1, 0, +1)
- Reward: -1 per step until success
- Max steps: 500
- Success: Swing up to goal height
- Success threshold: Episode reward > -100 (solve in < 100 steps)

QBound Configuration:
- V_min = -500 (worst case: never solve)
...
def train_agent(agent_type, num_episodes=1000, trajectory_length=2048):
    ...
    for episode in range(num_episodes):
        ...
        success_count.append(1 if episode_reward > -100 else 0)
...
def main():
    print("="*60)
    print("PPO on Acrobot-v1 (Discrete + Sparse Reward)")
    print("="*60)
    ...
    output_file = f"/root/projects/QBound/results/ppo/acrobot_{timestamp}.json"
