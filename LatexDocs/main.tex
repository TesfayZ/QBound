...
\author{
  Tesfay Zemuy Gebrekidan\thanks{ORCID: 0000-0002-0182-0997} \\
  Independent Researcher \\
  \texttt{tzemuy13@gmail.com} \\
  \url{https://github.com/TesfayZ/QBound}
}
...
\begin{abstract}
We present QBound, a stabilization mechanism for value-based reinforcement learning that derives and enforces Q-value bounds from known reward structures. QBound addresses overestimation bias \citep{thrun1993issues, van2016deep}---where bootstrapped Q-value estimates systematically exceed true values---by clipping next-state Q-values to environment-specific bounds $[Q_{\min}, Q_{\max}]$, which propagate naturally through temporal difference learning.
...
\textbf{Results:} For positive dense reward environments (e.g., CartPole: $r = +1$ per timestep), QBound achieves mean improvements of 12\% to 34\% across DQN variants, with CartPole Dueling showing 100\% win rate (5/5 seeds improve) and CartPole DQN showing 80\% win rate (4/5 seeds improve). For sparse reward environments (GridWorld, FrozenLake, MountainCar, Acrobot), QBound shows neutral results---the bounds are rarely violated during training, so clipping provides no benefit. For continuous control with negative rewards (Pendulum DDPG/TD3), soft clipping QBound preserves gradient flow while enforcing bounds.

This work provides: (1) systematic per-seed analysis across 40 runs showing QBound's effectiveness varies by algorithm (100\% win rate for Dueling DQN, 80\% for standard DQN), (2) analysis of why QBound is effective for dense positive rewards but neutral for sparse rewards, (3) honest reporting of seed-dependent failures even in successful cases, (4) clear recommendations based on win rates rather than mean improvements alone. Implementation imposes negligible overhead ($<$2\%). All experiments use 5 seeds with full reproducibility protocols.

\textbf{Recommendations:} (1) Use hard clipping QBound for positive dense rewards with Dueling DQN (100\% win rate, +22.5\% mean improvement). (2) For standard DQN on positive rewards, QBound helps on average (+12\%) but expect 20\% of seeds to degrade. (3) For sparse rewards, QBound provides no benefit as bounds are rarely violated. (4) For continuous control (DDPG/TD3), use soft clipping to preserve gradient flow. QBound is most effective for dense positive rewards where overestimation is prevalent.
\end{abstract}
...
\midrule
\textbf{Hard QBound} & Direct clipping: $Q \gets \clip(Q, Q_{\min}, Q_{\max})$ & Discrete actions & DQN, DDQN, Dueling \\
\textbf{Soft QBound} & Smooth clipping via softplus (preserves gradients) & Continuous actions & DDPG, TD3 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key distinction:} Hard QBound uses direct clipping on TD targets, while Soft QBound uses a smooth approximation that preserves gradient flow for actor-critic methods. Both enforce bounds algorithmically during training. Hard QBound succeeds for positive dense rewards (+12-34\%) where overestimation violations are frequent.
...
\textbf{Initialization bias and exploration:} \citet{kumar2020implicit} showed that network initialization creates implicit regularization in RL, affecting the learned Q-function. \citet{fortunato2017noisy} demonstrated that proper initialization and exploration mechanisms significantly impact learning dynamics.

\textbf{Value clipping approaches:} Most RL algorithms apply value clipping \textit{algorithmically} (e.g., PPO's value function clipping \citep{schulman2017proximal}, SAC's target entropy \citep{haarnoja2018soft}). Our work demonstrates that \textit{environment-aware bounds} derived from reward structure can provide targeted Q-value constraints. QBound implements this through hard clipping for discrete actions and penalty-based soft clipping for continuous actions.
...
\begin{enumerate}
    ...
    \item \textbf{Violation-based analysis:} We analyze when QBound is effective by measuring bound violation rates. For dense positive rewards, overestimation causes frequent violations (12.9\% on CartPole), making clipping beneficial. For sparse rewards, violations are rare ($<$1\%), explaining neutral results
    ...
    \item \textbf{Systematic evaluation:} We evaluate QBound across 8 classic control environments with 5 seeds each (40 runs), with explicit reporting of failure modes and implementation-dependent effectiveness
\end{enumerate}
...
\subsubsection{Sparse and Negative Rewards: QBound is Neutral}

For sparse rewards (MountainCar, Acrobot) and negative rewards (Pendulum), QBound shows neutral results. Our violation analysis reveals \textit{why}:
...
\paragraph{Why QBound is Neutral for Sparse/Negative Rewards.} Our experiments reveal that bound violation rates explain QBound effectiveness:
\begin{itemize}
    \item \textbf{CartPole (positive dense):} 12.9\% of Q-values violate bounds → clipping actively corrects overestimation → \textbf{+14\% improvement}
    \item \textbf{MountainCar (negative sparse):} 0.4\% of Q-values violate bounds → almost no correction needed → \textbf{neutral}
    \item \textbf{Acrobot (negative sparse):} 0.8\% of Q-values violate bounds → almost no correction needed → \textbf{neutral}
\end{itemize}

\textbf{Key insight:} For negative rewards, overestimation pushes Q-values toward 0 (less negative), which is \textit{within} the valid range $[Q_{\min}, 0]$. Since overestimated values don't violate bounds, clipping provides no correction. For positive rewards, overestimation pushes Q-values \textit{above} $Q_{\max}$, triggering clipping that stabilizes learning.

\subsubsection{Summary: When QBound Works}

\begin{table}[h]
\centering
\caption{QBound Effectiveness by Reward Type (5 seeds): Mean Change, Win Rate, and Violation Rate}
\label{tab:reward-sign-summary-theory}
\small
\begin{tabular}{llcccc}
\toprule
Reward Type & Algorithm & Mean $\Delta$ & Win Rate & Violation Rate & Status \\
\midrule
\textbf{Positive Dense} (CartPole) & DQN & +12.0\% & 4/5 (80\%) & 13.3\% & \textbf{Recommended} \\
\textbf{Positive Dense} (CartPole) & Dueling & +22.5\% & 5/5 (100\%) & 13.3\% & \textbf{Recommended} \\
\midrule
\textbf{Sparse Positive} (GridWorld) & DQN & -1.0\% & 1/5 (20\%) & 1.5\% & No benefit \\
\textbf{Sparse Positive} (FrozenLake) & DQN & -1.7\% & 3/5 (60\%) & 0.0\% & No benefit \\
\midrule
\textbf{Sparse Negative} (MountainCar) & DQN & -8.2\% & 1/5 (20\%) & 0.4\% & No benefit \\
\textbf{Sparse Negative} (Acrobot) & DQN & -4.9\% & 2/5 (40\%) & 0.8\% & No benefit \\
\midrule
\textbf{Negative Dense} (Pendulum) & DQN & -7.1\% & 2/5 (40\%) & 55.5\% & \textbf{Under investigation} \\
\textbf{Negative Dense} (Pendulum) & DDQN & -7.1\% & 2/5 (40\%) & 3.8\% & \textbf{Under investigation} \\
\textbf{Negative Dense} (Pendulum) & DDPG & +25.0\% & 2/5 (40\%) & 29.7\% & Mixed \\
\textbf{Negative Dense} (Pendulum) & TD3 & +15.3\% & 3/5 (60\%) & 3.5\% & Mixed \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Under Investigation:} DQN variants show degradation on dense negative rewards despite high violation rates. Pendulum DQN has 55.5\% violations (higher than CartPole's 13.3\%) yet QBound degrades performance by -7.1\%. Why high violations fail to benefit from clipping on dense negative rewards is under investigation.

\textbf{Recommendation:} Use Hard QBound for positive dense rewards where overestimation causes frequent bound violations. For sparse rewards, violations are rare so QBound provides no benefit. For negative dense rewards with DDPG/TD3, Soft QBound shows mixed results (+15\% to +25\% mean improvement but only 40-60\% win rate).
...
\textbf{Empirical status:} Soft QBound shows +25\% improvement for DDPG and +15\% for TD3 on Pendulum, but with high seed variance (see Section~\ref{sec:pendulum_ddpg}).
...
\textbf{Recommendation:} Use Hard QBound for discrete actions (DQN variants) with positive dense rewards. For continuous action methods (DDPG/TD3), use Soft QBound (penalty-based) to preserve gradient flow. QBound is designed for off-policy methods only; on-policy methods (e.g., PPO, A2C) do not suffer from the same overestimation dynamics and are outside QBound's scope.
...
This section discusses QBound's applicability to continuous control with actor-critic methods. Soft QBound (penalty-based) is required for DDPG/TD3 to preserve gradient flow.
...
For this reason, actor-critic methods require \textbf{Soft QBound}—implementing bounds through a penalty term that maintains gradient flow while discouraging Q-values from exceeding theoretical bounds. Unlike hard clipping, soft QBound uses: $\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{TD}} + \lambda \cdot (\max(0, Q-Q_{\max})^2 + \max(0, Q_{\min}-Q)^2)$.

\subsubsection{Soft QBound Results (5-Seed Evaluation)}

Soft QBound with DDPG and TD3 on Pendulum-v1 shows mixed results with high variance:

\begin{table}[H]
\centering
\caption{Pendulum-v1 DDPG/TD3: Soft QBound Results (5-seed mean $\pm$ std)}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Method} & \textbf{Baseline} & \textbf{With Soft QBound} & \textbf{Change} \\
\midrule
DDPG & $-213.1 \pm 89.3$ & $-159.8 \pm 11.7$ & \textcolor{green}{+25.0\%} \\
TD3 & $-202.4 \pm 71.9$ & $-171.5 \pm 34.9$ & \textcolor{green}{+15.3\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{DDPG:} +25.0\% mean improvement but only 2/5 seeds improved (40\% win rate). Individual seeds ranged from +61.5\% to -4.2\%.
    \item \textbf{TD3:} +15.3\% mean improvement with 3/5 seeds improved (60\% win rate). Individual seeds ranged from +53.6\% to -57.7\%.
    \item \textbf{High variance:} Both methods show substantial variance, suggesting Soft QBound effectiveness is seed-dependent on negative reward environments.
\end{itemize}

\textbf{Violation Rates (Soft QBound):}

\begin{table}[H]
\centering
\caption{Pendulum DDPG/TD3: Violation Rates by Seed (\%)}
\small
\begin{tabular}{@{}lccccc|c@{}}
\toprule
\textbf{Method} & \textbf{Seed 42} & \textbf{Seed 43} & \textbf{Seed 44} & \textbf{Seed 45} & \textbf{Seed 46} & \textbf{Mean} \\
\midrule
...
\textbf{Note:} Violation rates vary across algorithms: Pendulum DQN shows 55.5\%, DDPG 29.7\%, TD3 3.5\%.

\textbf{Comparison with Hard QBound on DQN:} Hard QBound degrades Pendulum DQN by -7.1\% despite 55.5\% violation rate. Soft QBound on DDPG/TD3 shows +15\% to +25\% improvement. Why DQN variants fail on dense negative rewards despite high violations is under investigation.
...
\paragraph{Pendulum DQN (Discrete Actions): Static QBound Results}

\textbf{Configuration:} Discretized action space (5 bins), $Q_{\min} = -1800$, $Q_{\max} = 0$

\begin{table}[h]
\centering
\caption{Pendulum DQN: Static QBound Results (5 seeds)}
...
\textbf{Key Finding:} For negative reward environments, static QBound shows degraded performance compared to baselines despite high violation rates.

\textbf{Violation Rates (Pendulum DQN/DDQN):}
\begin{itemize}
    \item Pendulum DQN: 55.5\% mean violation rate (seeds 43-45: 51\%--62\%)
    \item Pendulum DDQN: 3.8\% mean violation rate (seeds 43-45: 2\%--7\%)
\end{itemize}

\textbf{Interpretation:} Despite high violation rates on Pendulum DQN (55.5\%), QBound still degrades performance (-7.1\%). This contradicts the expectation that high violations would benefit from clipping. Why DQN variants fail on dense negative rewards despite high violations is under investigation.
...
\textbf{Continuous Control (DDPG/TD3):} For continuous action spaces, Soft QBound (penalty-based) is required to preserve gradient flow. Soft QBound shows +25\% (DDPG) and +15\% (TD3) mean improvement on Pendulum, but with high seed variance.
...
\midrule
\multirow{3}{*}{Negative Dense} & Pendulum DQN & -3.3\% & 2/5 (40\%) & No \\
& Pendulum DDPG (Soft) & \textbf{+25.0\%} & 2/5 (40\%) & No \\
& Pendulum TD3 (Soft) & \textbf{+15.3\%} & 3/5 (60\%) & No \\
\midrule
...
\begin{itemize}
    ...
    \item \textcolor{orange}{$\sim$} \textbf{Mostly reliable (1/8):} CartPole DQN shows 80\% win rate, but individual seeds can fail
    \item $\times$ \textbf{Unreliable (6/8):} Win rates of 20-60\% indicate QBound performs no better than chance. However, DDPG and TD3 with Soft QBound show strong mean improvements (+25\%, +15\%) despite low win rates, driven by large gains on specific seeds.
\end{itemize}
...
& Pendulum DDPG (Soft) & 2/5 (40\%) & \textbf{+25.0\%} & No \\
& Pendulum TD3 (Soft) & 3/5 (60\%) & \textbf{+15.3\%} & No \\
\midrule
...
\begin{itemize}
    ...
    \item \textbf{CartPole DQN shows 80\% win rate} but has one seed with degradation (seed 44: -8.7\%).
    \item \textbf{Soft QBound on DDPG/TD3 shows high variance but strong mean improvements.} DDPG ranges from +61.5\% (seed 42) to -4.2\% (seed 46); TD3 ranges from +53.6\% (seed 43) to -57.7\% (seed 44). Large gains on specific seeds drive the positive means despite low win rates.
...
\begin{enumerate}
    \item \textbf{PRIMARY: Value-based methods with discrete actions} (DQN, Double-Q, Dueling DQN)
        ...
        \end{itemize}
    \item \textbf{MIXED RESULTS: Off-policy actor-critic} (DDPG, TD3)
        \begin{itemize}
            \item Soft QBound (penalty-based) required to preserve gradient flow
            \item DDPG: +25\% mean, TD3: +15\% mean on Pendulum (high seed variance)
        \end{itemize}
    \item \textbf{AVOID: Hard QBound with continuous actions}
        \begin{itemize}
            \item Hard clipping disrupts smooth policy gradients
            \item Must use Soft QBound (penalty-based) for actor-critic methods
        \end{itemize}
\end{enumerate}
...
\textbf{2. Reward Sign Dependence:} QBound's effectiveness fundamentally depends on reward sign (Section~\ref{subsec:multiseed-results}). For negative rewards, QBound causes performance degradation: Pendulum DQN (-7.1\%), MountainCar DDQN (-47.4\%), Acrobot (-3.6\%). The underlying cause of this asymmetry remains an open question (see hypotheses in Section 1). This failure rate demonstrates QBound is \textit{not universally beneficial}. Success is limited to positive dense rewards (CartPole: +12-34\%). Practitioners must analyze reward structure before applying QBound.
...
\textbf{4. Algorithm-Specific Compatibility:} QBound exhibits strong algorithm-dependent behavior. For off-policy actor-critic methods (DDPG, TD3), Soft QBound shows +25\% (DDPG) and +15\% (TD3) mean improvement on Pendulum, but with high seed variance (40-60\% win rate). Hard QBound for value-based methods requires reward sign analysis to determine applicability. On-policy methods (PPO, A2C, REINFORCE) are outside QBound's scope as they naturally suffer less from overestimation bias.
...
\begin{enumerate}
    ...
    \item \textbf{Algorithmic Generalization:} Validation across DQN, Double DQN, and Dueling DQN on CartPole, with consistent improvements (+12\% to +33.6\%) across architectures
...
\begin{itemize}
    ...
    \item \textbf{Negative dense rewards (Pendulum DQN):} -3\% to -7\% degradation. QBound consistently harms performance on negative reward environments, where explicit bounds interfere with learning dynamics. We discuss the asymmetry and candidate mechanisms in Section~\ref{subsec:reward-sign-dependence} and Limitations.

    \item \textbf{Continuous control (DDPG/TD3):} Soft QBound shows +25\% (DDPG) and +15\% (TD3) mean improvement on Pendulum, but with high seed variance. Unlike Hard QBound on DQN which degrades, Soft QBound shows promise for negative reward continuous control.
...
\textbf{Overall success rate:} 40\% (6/15 algorithm-environment combinations show $>$10\% improvement), 13\% neutral, 47\% degradation. \textbf{Key insight:} QBound's effectiveness shows strong reward-sign dependence: positive rewards yield consistent benefits, negative rewards yield consistent degradation.
...
DDPG/TD3 & Soft (penalty) & Negative rewards (high variance) & Pendulum: +25\% (DDPG), +15\% (TD3) \\
...
\begin{enumerate}
    \item \textbf{Choose the right QBound type:}
        \begin{itemize}
            ...
            \item \textit{Soft QBound (penalty-based):} Use for continuous action spaces (DDPG, TD3) to preserve gradient flow
    ...
    \item \textbf{Primary use cases (highest benefit):}
        \begin{itemize}
            ...
            \item Soft QBound + DDPG/TD3: Pendulum +25\% (DDPG), +15\% (TD3) with high variance
        \end{itemize}
...
For the reinforcement learning community, QBound offers a practical tool that can be immediately applied to existing algorithms with minimal modification. \textbf{Our 5-seed evaluation across 8 classic control environments shows QBound is most effective for positive dense reward environments}, achieving consistent improvements on CartPole (+12\% to +33.6\% across DQN variants) with Dueling DQN showing 100\% win rate. QBound provides no benefit on sparse terminal rewards and degrades performance on negative reward environments.
...
\begin{enumerate}
    ...
    \item \textbf{Not universally beneficial:} QBound works best for positive dense rewards but causes degradation on negative reward environments
...
\begin{itemize}
    ...
    \item \textbf{Continuous action compatibility:} QBound for actor-critic methods requires Soft QBound (penalty-based) to maintain gradient flow:
        \begin{itemize}
            \item \textit{Hard clipping:} Incompatible—disrupts critic gradients required for policy learning
            \item \textit{Soft QBound:} Shows +25\% (DDPG), +15\% (TD3) on Pendulum with high variance
        \end{itemize}
    ...
    \item \textbf{Algorithm-specific tuning required:} QBound shows strong algorithm dependence:
        \begin{itemize}
            ...
            \item \textit{Mixed:} DDPG/TD3 with Soft QBound (+15-25\% Pendulum, high variance)
            \item \textit{Fails:} DQN on negative rewards
        \end{itemize}
...
\textbf{Bottom line:} QBound provides consistent improvements (+12\% to +33.6\%) on positive dense reward environments like CartPole, with Dueling DQN showing 100\% win rate across 5 seeds. For continuous control with negative rewards, Soft QBound shows promise (+15-25\% on Pendulum DDPG/TD3) but with high seed variance. Hard QBound fails on negative reward DQN environments and provides no benefit on sparse terminal rewards. Apply QBound selectively based on reward structure and action space.
