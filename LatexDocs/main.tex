...
\textbf{Results:} (1) \textit{Positive dense rewards:} Recommended (+12\% to +33\%, 80-100\% win rate on CartPole). (2) \textit{Sparse rewards:} Neutral/No benefit (violations rare: 0-1.5\%). (3) \textit{Negative dense + DDPG/TD3:} Mixed (+15\% to +25\% mean, but only 40-60\% win rate). (4) \textit{Negative dense + DQN variants:} Degrading (-7.1\%, under investigation).
...
\textbf{Recommendations:} (1) Use hard clipping QBound for positive dense rewards with Dueling DQN (100\% win rate, +22.5\% mean). (2) For sparse rewards, QBound provides no benefit as bounds are rarely violated. (3) For negative dense rewards with DDPG/TD3, soft clipping shows mixed results (40-60\% win rate). (4) Avoid hard clipping QBound for negative dense rewards with DQN variants (causes degradation).
\end{abstract}
...
\begin{itemize}
    \item \textbf{Positive dense rewards:} Recommended (+12\% to +33\%, 80-100\% win rate)
    \item \textbf{Sparse rewards:} Neutral/No benefit (violations rare: 0-1.5\%)
    \item \textbf{Negative dense + DDPG/TD3:} Mixed (+15\% to +25\% mean, but only 40-60\% win rate)
    \item \textbf{Negative dense + DQN variants:} Degrading (-7.1\%, under investigation)
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Reward-dependent effectiveness:} QBound shows varying effectiveness across reward structures:
    \begin{itemize}
        \item Positive dense rewards: Recommended (+12\% to +33\%, 80-100\% win rate)
        \item Sparse rewards: Neutral/No benefit (violations rare: 0-1.5\%)
        \item Negative dense + DDPG/TD3: Mixed (+15\% to +25\% mean, but only 40-60\% win rate)
        \item Negative dense + DQN variants: Degrading (-7.1\%, under investigation)
    \end{itemize}
