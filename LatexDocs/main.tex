...
\begin{table}[h]
\centering
\caption{QBound Effectiveness by Reward Type (5 seeds)}
\label{tab:reward-sign-summary-theory}
\begin{tabular}{llccc}
\toprule
Reward Type & Algorithm & Violation Rate & QBound Effect & Recommendation \\
\midrule
\textbf{Positive Dense} (CartPole) & DQN & 13.3\% & +12\% to +34\% & \textbf{Use Hard QBound} \\
\midrule
\textbf{Sparse Positive} (GridWorld) & DQN & 1.5\% & Neutral & No benefit \\
\textbf{Sparse Positive} (FrozenLake) & DQN & 0.0\% & Neutral & No benefit \\
\midrule
\textbf{Sparse Negative} (MountainCar) & DQN & 0.4\% & Neutral & No benefit \\
\textbf{Sparse Negative} (Acrobot) & DQN & 0.8\% & Neutral & No benefit \\
\midrule
\textbf{Negative Dense} (Pendulum) & DQN & 55.5\% & -7.1\% & No benefit \\
\textbf{Negative Dense} (Pendulum) & DDQN & 3.8\% & -7.1\% & No benefit \\
\textbf{Negative Dense} (Pendulum) & DDPG & 29.7\% & +25.0\% & Under investigation \\
\textbf{Negative Dense} (Pendulum) & TD3 & 3.5\% & +15.3\% & Under investigation \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Observations:}
\begin{itemize}
    \item Positive dense (CartPole DQN): 13.3\% violation rate
    \item Negative dense (Pendulum DQN): 55.5\% violation rate
    \item Negative dense (Pendulum DDQN): 3.8\% violation rate
    \item Negative dense (Pendulum DDPG): 29.7\% violation rate
    \item Negative dense (Pendulum TD3): 3.5\% violation rate
    \item Sparse rewards (all): 0.0\% -- 1.5\% violation rate
\end{itemize}

\textbf{Note:} Violation rates vary significantly across algorithms even on the same environment. Pendulum DQN shows higher violation rates (55.5\%) than CartPole DQN (13.3\%), while Pendulum DDQN shows lower rates (3.8\%). The relationship between violation rate and performance is under investigation.

\textbf{Recommendation:} Use Hard QBound for positive dense rewards where overestimation causes frequent bound violations. For sparse rewards, violations are rare so QBound provides no benefit. For negative dense rewards with continuous control (DDPG/TD3), Soft QBound shows promise but results are under investigation.
...
\textbf{Note:} Unlike DQN on negative rewards ($<$1\% violation rate), DDPG/TD3 show higher and more variable violation rates. Why violation rates differ across algorithms and seeds on negative reward environments is under investigation.

\textbf{Comparison with Hard QBound on DQN:} While Hard QBound degrades Pendulum DQN by -7.1\%, Soft QBound shows mean improvement for DDPG/TD3. Why this difference occurs is under investigation.
...
\textbf{Interpretation:} Despite high violation rates on Pendulum DQN (55.5\%), QBound still degrades performance (-7.1\%). This contradicts the expectation that high violations would benefit from clipping. The relationship between violation rate and performance is under investigation.
...
\begin{enumerate}
    ...
    \item \textbf{Algorithmic Generalization:} Validation across different DQN architectures (standard DQN, Double DQN, Dueling DQN), demonstrating applicability within the value-based off-policy domain
...
\textbf{Repository:} \url{https://github.com/TesfayZ/QBound}
