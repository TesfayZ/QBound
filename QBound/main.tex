...
\begin{abstract}
Value-based reinforcement learning methods suffer from \textbf{overestimation bias} \citep{thrun1993issues, van2016deep}—bootstrapped Q-value estimates systematically exceed true values, causing instability and poor sample efficiency. We present \textbf{QBound}, a principled stabilization mechanism that exploits environment structure to prevent overestimation. \textbf{Core contribution:} When reward bounds are known (common in many domains), QBound derives and enforces corresponding Q-value bounds, preventing unbounded overestimation while preserving valid Q-values. This \textit{environment-aware bounding} stabilizes temporal difference learning \citep{sutton2018reinforcement}, reducing variance and accelerating convergence. For sparse reward environments, we use static bounds; for dense reward environments, we introduce step-aware dynamic bounds that adapt to remaining episode potential. \textbf{Key insight:} By preventing overestimation at its source (bootstrapped targets), QBound addresses the root cause of instability, yielding substantial sample efficiency improvements without catastrophic failures. QBound applies to value-based methods with discrete actions (DQN, Double-Q) and generalizes across different network architectures (standard, Dueling). We provide theoretical analysis of sample complexity improvements and convergence properties. Empirical evaluation across seven diverse environments demonstrates consistent improvements: \textbf{LunarLander} sparse-reward continuous control task achieves \textbf{+263.9\% improvement} with QBound+Double DQN reaching 83\% success rate (228.0 $\pm$ 89.6 reward, Standard DQN architecture) versus baseline's 11\% (-61.8 $\pm$ 177.6), GridWorld (20.2\% faster convergence), FrozenLake (76\% better final performance), CartPole (14.2\% improvement in comprehensive 6-way evaluation), with negligible computational overhead ($<2\%$). \textbf{Critical finding:} Comprehensive comparison with Double DQN across seven environments reveals that \textit{pessimistic Q-learning is fundamentally environment-dependent}—Double DQN catastrophically fails on dense-reward, long-horizon tasks (CartPole: -76.3\%) while succeeding on sparse-reward tasks (LunarLander: +400.5\%). QBound's environment-aware bounds provide a more robust alternative to algorithm-level pessimism, achieving improvements in 2/4 evaluated environments (average +63.5\%) without catastrophic failures. \textbf{Critical finding on continuous actions:} QBound's applicability to continuous action spaces depends fundamentally on implementation. \textit{Hard QBound} (direct clipping) is incompatible with actor-critic methods—disrupting smooth critic gradients causes 893\% degradation on Pendulum DDPG. However, \textit{Soft QBound} (penalty-based) \textbf{successfully extends to continuous control}, achieving \textbf{+712\% improvement} when replacing target networks in simple DDPG (Pendulum: -1465 → -206) and \textbf{+5\% enhancement} over standard DDPG (-181 → -172, best performance). This demonstrates QBound can \textit{partially replace target network stabilization} in continuous control. However, Soft QBound conflicts with TD3's clipped double-Q mechanism (-180 → -1259), demonstrating that algorithm-specific interactions require careful consideration. Extension to policy gradient methods (PPO) reveals \textit{nuanced effectiveness}—succeeds on continuous sparse rewards (LunarLanderContinuous: +34.2\%, variance reduced 55\%) and discrete dense with dynamic bounds (CartPole: +17.9\%), but degrades performance on continuous dense tasks (Pendulum: -26.9\%) and conflicts with GAE on discrete sparse tasks (LunarLander: -30.9\%). \textbf{Recommendation:} Use Hard QBound for discrete actions (DQN variants), Soft QBound for continuous actions (DDPG, selected PPO tasks).
\end{abstract}
...
\textbf{Key Observation:} The sample efficiency hierarchy correlates directly with whether methods learn value functions. This suggests that improving value function learning improves sample efficiency across the entire spectrum of methods that use critics. \textbf{However, value-based methods face a fundamental challenge:} bootstrapped Q-value estimates suffer from instability and overestimation bias \citep{thrun1993issues, van2016deep}, which directly undermines their sample efficiency advantage. Stabilizing value learning is therefore critical for achieving better sample efficiency.
...
We propose \textbf{QBound}, a stabilization mechanism that prevents overestimation bias by exploiting known environment structure. \textbf{Core contribution:} QBound addresses the root cause of instability in bootstrapped value learning—unbounded overestimation—by deriving and enforcing environment-aware Q-value bounds. Unlike generic pessimism (e.g., Double-Q \citep{van2016deep}), QBound's bounds are \textit{environment-specific}, providing stabilization without excessive conservatism.
...
\begin{itemize}
    ...
    \item Works with any algorithm that learns Q-functions or critics (DQN, DDPG, PPO)
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Bootstrapping-based enforcement:} QBound leverages the natural propagation of bootstrapped targets through temporal difference learning \citep{sutton2018reinforcement}, requiring only simple clipping operations
    ...
    \item \textbf{Architectural generalization:} QBound validates generalization across architecturally distinct variants (standard DQN, Dueling DQN) and algorithm classes (DQN, Double DQN, PPO)
    \item \textbf{Comprehensive evaluation:} We provide extensive analysis across seven diverse environments, including honest reporting of failure modes (continuous control, exploration-critical tasks)
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Hard QBound:} 893\% performance degradation (catastrophic failure)
    \item \textbf{Soft QBound:} +712\% improvement over baseline, +5\% over standard DDPG
\end{itemize}

\textbf{Recommendation:} Use Hard QBound for discrete actions (DQN variants), Soft QBound for continuous actions (DDPG, selected TD3/PPO configurations). Never use Hard QBound with continuous action actor-critic methods.
...
\begin{itemize}
    ...
    \item \textbf{Examples:} DDPG, TD3, PPO with continuous actions
...
\begin{itemize}
    ...
    \item \textbf{Example:} CartPole with dynamic bounds achieved +17.9\% vs +0.4\% with static bounds in PPO experiments
...
\caption{CartPole 6-way comparison (500 episodes). Baseline DQN achieves best performance (183K total reward, 366 avg), while Double DQN catastrophically fails (43K, 87 avg) - a 76.3\% degradation demonstrating that pessimistic Q-learning is fundamentally incompatible with dense-reward, long-horizon tasks.}
...
\textbf{Critical Finding:} Double DQN's pessimistic Q-learning causes catastrophic failure on CartPole (dense reward, long horizon), achieving only 86.80 avg reward compared to baseline DQN's 366.04 (-76.3\%). This demonstrates that algorithm-level pessimism is fundamentally environment-dependent.
...
\begin{enumerate}
    \item \textbf{Double DQN is environment-dependent:} Excels on sparse rewards (LunarLander: +400.5\% vs baseline DQN) but catastrophically fails on dense rewards (CartPole: -76.3\%)
    \item \textbf{QBound provides robust improvements:} Never causes catastrophic failures, achieves best or near-best performance in 2/4 environments (GridWorld, LunarLander)
...
\textbf{Critical Finding:} QBound's applicability to continuous control depends fundamentally on implementation—\textit{Hard QBound} (clipping) catastrophically fails, while \textit{Soft QBound} (penalty) successfully stabilizes and enhances DDPG.
...
\caption{Pendulum-v1: 6-way DDPG/TD3 comparison with Soft QBound. \textbf{Left panels:} Baseline methods. Simple DDPG (orange) without target networks fails catastrophically. \textbf{Right panels:} Soft QBound dramatically improves Simple DDPG (orange, middle-right) achieving similar performance to methods with target networks, and enhances Standard DDPG (purple, bottom-left) to best overall performance. TD3 + Soft QBound fails catastrophically (brown, bottom-right), indicating conflicts with TD3's double-Q clipping mechanism. Training over 500 episodes, 20-episode smoothing.}
...
Soft QBound brings Simple DDPG from catastrophic failure to near-competitive performance with target network methods, demonstrating that \textit{value bounding can serve as an alternative stabilization mechanism} in continuous control.
...
\subsection{Part 5: Policy Gradient Methods - PPO with Value Bounding}
...
% which distorted advantages and caused catastrophic failures.
% New implementation (Option 1): Clip RETURNS only (targets for critic), NOT during GAE.
% This matches DQN/DDQN philosophy: clip what the value network learns from.
% Re-run experiments after fixing ppo_qbound_agent.py
%
% IMPORTANT DISTINCTION: QBound for PPO is NOT about overestimation prevention
% PPO doesn't have systematic overestimation bias (no max operator in bootstrapping)
% Instead, QBound for PPO targets: variance reduction, gradient scaling, regularization
% This is a DIFFERENT motivation than DQN/DDQN where overestimation is the core problem

Having established that Soft QBound successfully extends to DDPG but conflicts with TD3 for continuous control, we investigated whether \textit{policy gradient methods} could benefit from value bounding. Unlike deterministic actor-critic methods, PPO (Proximal Policy Optimization) \citep{schulman2017proximal} uses a critic V(s) for advantage estimation rather than Q(s,a) for policy gradients, potentially avoiding the gradient discontinuity issues that affect DDPG/TD3.

\textbf{Important Distinction:} QBound's application to PPO differs fundamentally from its application to value-based methods. In DQN/DDQN, QBound addresses \textit{systematic overestimation bias} caused by the max operator in bootstrapping ($Q(s,a) \gets r + \gamma \max_{a'} Q(s',a')$). However, PPO does not suffer from this bias—its value function bootstraps without a max operator ($V(s) \gets r + \gamma V(s')$), and policy updates depend on advantages rather than value magnitudes. Therefore, QBound for PPO targets \textit{secondary stabilization mechanisms}: variance reduction in critic targets, better-scaled gradients during early training, and soft regularization of value predictions. This represents an exploratory application of environment-aware bounds beyond overestimation prevention.

\subsubsection{Theoretical Motivation}

\textbf{Why PPO is Different from Value-Based Methods:}
\begin{itemize}
    \item \textbf{DQN/DDQN:} Use $Q(s,a) \gets r + \gamma \max_{a'} Q(s',a')$ (max operator causes overestimation bias)
    \item \textbf{PPO:} Uses $V(s) \gets r + \gamma V(s')$ (no max operator, no systematic overestimation)
    \item \textbf{Policy updates:} PPO uses advantages $A(s,a)$, not value magnitudes, for policy gradients
\end{itemize}

\textbf{Hypothesis:} While PPO doesn't suffer from overestimation bias, environment-aware bounds may provide \textit{secondary benefits}:
\begin{enumerate}
    \item \textbf{Variance reduction:} Clipping critic targets may reduce bootstrap variance during early training
    \item \textbf{Gradient scaling:} Bounded values prevent extreme predictions that cause training instability
    \item \textbf{Regularization:} Acts as soft constraint on critic, potentially improving generalization
\end{enumerate}

\textbf{Implementation (Corrected):} We apply QBound to PPO's critic targets (not advantages):
\begin{enumerate}
    \item Compute GAE \textit{without} clipping: $\delta_t = r + \gamma V(s') - V(s)$ (preserves unbiased advantages)
    \item Compute returns: $G_t = A_t + V(s_t)$
    \item \textbf{Clip returns only:} $G_t^{\text{clipped}} = \clip(G_t, V_{\min}, V_{\max})$ (bounds what critic learns)
    \item Train critic with clipped returns: $\mathcal{L}_{\text{critic}} = \text{MSE}(V(s), G^{\text{clipped}})$
    \item Train policy with \textit{unclipped} advantages: $\mathcal{L}_{\text{policy}} = -A \cdot \log \pi(a|s)$
\end{enumerate}
...
\textbf{QBound Configuration for PPO:}
...
\begin{tabular}{@{}lccccl@{}}
\toprule
\textbf{Environment} & \textbf{$V_{\min}$} & \textbf{$V_{\max}$} & \textbf{$\gamma$} & \textbf{Type} & \textbf{Implementation} \\
\midrule
Pendulum-v1 & -1409.33 & 0 & 0.99 & Static/Dynamic & Hard clip returns \\
\bottomrule
\end{tabular}
\caption{PPO value bounding configuration. Uses hard clipping on returns (critic targets): $G^{\text{clip}} = \clip(G, V_{\min}, V_{\max})$. For dynamic bounds, $V_{\min}(t) = r_{\text{step}} \cdot (1 - \gamma^{H-t}) / (1-\gamma)$ adapts to remaining episode potential. Static bound $V_{\min} = -1409.33$ is the discounted worst-case cumulative reward (200 steps × -16.27 avg reward, with $\gamma=0.99$).}
\label{tab:ppo-qbound-config}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/ppo_continuous_comparison.png}
\caption{PPO + QBound on continuous action tasks: LunarLanderContinuous-v3 (left) shows +34.2\% improvement with reduced variance, while Pendulum-v1 (right) exhibits significant degradation (-26.9\%), demonstrating environment-specific compatibility. Training over 500 episodes with 20-episode smoothing.}
\label{fig:ppo-continuous}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{PPO on LunarLanderContinuous:} Baseline 116.7 (13\% success) → PPO+QBound 152.5 (9\% success) = \textcolor{green}{+31\%}
\end{itemize}
...
\textbf{2. Environment-Dependent Failure Modes:} QBound degrades performance on exploration-critical environments (MountainCar: -16.6\%, Acrobot: -7.6\%), demonstrating it is \textit{not universally beneficial}. Over-constraining Q-values during early exploration may prevent discovery of high-value trajectories. Practitioners must assess environment characteristics before applying QBound.
...
\textbf{4. Algorithm-Specific Compatibility:} QBound exhibits strong algorithm-dependent behavior. Hard QBound is incompatible with deterministic actor-critic methods (Pendulum DDPG: -893\% degradation) due to gradient disruption. Soft QBound conflicts with TD3's clipped double-Q (-700\% degradation) and GAE in discrete sparse PPO tasks (-30.9\%). Careful algorithm-specific consideration is required.
...
We presented \textbf{QBound}, a stabilization mechanism that prevents overestimation bias in bootstrapped value learning. By enforcing environment-specific Q-value bounds derived from reward structure, QBound addresses the root cause of instability in temporal difference methods \citep{sutton2018reinforcement, thrun1993issues}. This principled approach to preventing unbounded overestimation yields substantial sample efficiency improvements (5-31\% across diverse environments, up to 712\% for continuous control with Soft QBound) without the catastrophic failures observed in generic pessimistic methods like Double-Q \citep{van2016deep}.
...
\begin{enumerate}
    ...
    \item \textbf{Architectural and Algorithmic Generalization:} Validation across different architectures (standard DQN, Dueling DQN) and algorithm families (DQN, DDPG, TD3, PPO), demonstrating broad applicability
    \item \textbf{Critical Comparative Analysis:} Direct comparison with Double DQN revealing environment-dependent behavior of generic pessimism—Double DQN fails catastrophically on dense-reward tasks (-66\% on CartPole) while QBound's environment-aware bounds succeed universally
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{Dense rewards (CartPole):} Double DQN -76.3\%, QBound +14.2\% → pessimism hurts
    ...
    \item \textbf{Architectural Generalization:} QBound generalizes across different DQN architectures, validating its architectural-agnostic design:
    \begin{itemize}
        \item \textit{Dueling DQN:} +95.9\% improvement on LunarLander (201.71 vs 102.95 baseline), achieving 77\% success rate
        \item \textit{Complementarity:} QBound provides substantial gains even when Dueling's stronger baseline already improves over standard DQN
        \item \textit{Variance reduction:} 34.7\% reduction in standard deviation (198.57 $\to$ 130.00)
    \end{itemize}
...
\begin{table}[h]
\centering
\caption{Algorithm-Specific QBound Recommendations}
\small
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Algorithm} & \textbf{QBound Type} & \textbf{Effectiveness} & \textbf{Key Result} \\
\midrule
\multicolumn{4}{@{}l}{\textit{\textbf{Value-Based (Discrete Actions):}}} \\
DQN & Hard & Excellent & LunarLander: +263.9\% \\
Double DQN & Hard & Excellent & LunarLander: +83\% success \\
Dueling DQN & Hard & Excellent & LunarLander: +95.9\% \\
\midrule
\multicolumn{4}{@{}l}{\textit{\textbf{Actor-Critic (Continuous Actions):}}} \\
DDPG (no targets) & Soft & Excellent & Pendulum: +712\% vs baseline \\
DDPG (standard) & Soft & Good & Pendulum: +5\% enhancement \\
TD3 & Soft & \textbf{Fails} & Pendulum: -600\% \\
DDPG/TD3 & Hard & \textbf{Never use} & Pendulum: -893\% \\
\midrule
\multicolumn{4}{@{}l}{\textit{\textbf{Policy Gradient:}}} \\
PPO (continuous sparse) & Soft & Good & LunarLander-Cont: +34.2\% \\
PPO (discrete dense, dynamic) & Soft & Moderate & CartPole: +17.9\% \\
PPO (discrete sparse) & Soft & \textbf{Fails} & LunarLander: -30.9\% \\
PPO (continuous dense) & Soft & \textbf{Fails} & Pendulum: -26.9\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Choose the right QBound type:}
        \begin{itemize}
            ...
            \item \textit{Soft QBound (penalty):} Use for continuous action spaces (DDPG, selected PPO)
    ...
    \item \textbf{Primary use cases (highest benefit):}
        \begin{itemize}
            ...
            \item Soft QBound + DDPG: Continuous control, can replace target networks (+712\%)
            \item Soft QBound + PPO: Continuous action, sparse rewards (+34.2\%)
        \end{itemize}

    \item \textbf{Algorithm-specific warnings:}
        \begin{itemize}
            \item \textit{TD3:} Soft QBound conflicts with clipped double-Q (-600\% on Pendulum)
            \item \textit{PPO + discrete sparse:} Conflicts with GAE (-30.9\% on LunarLander)
            \item \textit{DDPG/TD3 + Hard QBound:} Catastrophic failure (-893\%)
        \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Algorithm-specific tuning required:} QBound shows strong algorithm dependence:
        \begin{itemize}
            \item \textit{Excellent:} DQN variants (all), DDPG with Soft QBound
            \item \textit{Good:} PPO on continuous sparse rewards
            \item \textit{Fails:} TD3 (conflicts with double-Q), PPO on discrete sparse (conflicts with GAE)
        \end{itemize}

    \item \textbf{Environment characteristics matter:} Not universally beneficial:
        \begin{itemize}
            \item \textit{Best:} Sparse rewards with known bounds (+263.9\% LunarLander, +712\% Pendulum DDPG)
            \item \textit{Moderate:} Dense rewards with dynamic bounds (+17.9\% CartPole PPO)
            \item \textit{Hurts:} Exploration-critical tasks (MountainCar: -16.6\%, Acrobot: -7.6\%)
        \end{itemize}
