...
\begin{itemize}
    ...
    \item \textbf{Outcome:} 5-31\% improvement in sample efficiency and cumulative reward across diverse environments
...
\caption{CartPole 6-way comparison (500 episodes). Baseline DQN achieves best performance (183K total reward, 366 avg), while Double DQN catastrophically fails (43K, 87 avg) - a 21.3\% degradation demonstrating that pessimistic Q-learning is fundamentally incompatible with dense-reward, long-horizon tasks.}
...
\textbf{Critical Finding:} Double DQN's pessimistic Q-learning causes catastrophic failure on CartPole (dense reward, long horizon), achieving only 86.80 avg reward compared to baseline DQN's 366.04 (-21.3\%). This demonstrates that algorithm-level pessimism is fundamentally environment-dependent.
...
\begin{enumerate}
    \item \textbf{Double DQN is environment-dependent:} Excels on sparse rewards (LunarLander: +400.5\% vs baseline DQN) but catastrophically fails on dense rewards (CartPole: -21.3\%)
...
LunarLanderContinuous-v3 & Sparse & 116.74 $\pm$ 85.34 & 152.47 $\pm$ 41.50 & \textcolor{green}{+30.6\%} \\
Pendulum-v1 & Dense & -461.28 $\pm$ 228.01 & -1210.22 $\pm$ 65.83 & \textcolor{red}{-162.4\%} \\
\midrule
...
\caption{PPO + QBound on continuous action tasks: LunarLanderContinuous-v3 (left) shows +30.6\% improvement with reduced variance, while Pendulum-v1 (right) exhibits catastrophic failure (-162\%), demonstrating environment-specific compatibility. Training over 500 episodes with 20-episode smoothing.}
...
\begin{itemize}
    \item \textbf{Continuous + Sparse (LunarLanderContinuous):} \textcolor{green}{+30.6\%} improvement, variance reduced 51\% (85.3 → 41.5)
    \item \textbf{Continuous + Dense (Pendulum):} \textcolor{red}{-162.4\%} \textbf{catastrophic failure}, over-constrains value function
...
\textbf{Key Pattern:} QBound helps PPO on \textit{continuous action spaces with sparse rewards}, where value stabilization benefits exploration. It \textbf{catastrophically fails} on continuous dense rewards (Pendulum -162\%), mirroring the TD3+QBound failure pattern, suggesting fundamental conflicts between QBound and certain algorithm-task combinations.
