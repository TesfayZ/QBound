...
We evaluated PPO+QBound on two representative environments:

\begin{table}[h]
\centering
\caption{PPO + QBound Results (Final 100 Episodes)}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Environment} & \textbf{Type} & \textbf{Baseline PPO} & \textbf{PPO+QBound} & \textbf{Change} \\
\midrule
CartPole-v1 (Static) & Discrete + Dense & 210.22 $\pm$ 135.54 & 211.02 $\pm$ 149.28 & +0.4\% \\
CartPole-v1 (Dynamic) & Discrete + Dense & 210.22 $\pm$ 135.54 & 247.81 $\pm$ 125.48 & \textcolor{green}{+17.9\%} \\
LunarLander-v3 & Discrete + Sparse & 219.20 $\pm$ 111.84 & 151.55 $\pm$ 110.26 & \textcolor{red}{-30.9\%} \\
\midrule
\multicolumn{5}{@{}l}{\textit{Success Rates (LunarLander, reward > 200):}} \\
~ & ~ & 80.0\% & 38.0\% & \textcolor{red}{-42\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{1. Algorithm-Dependent Effectiveness:}

QBound's performance varies dramatically by algorithm:
\begin{itemize}
...
\textbf{2. Dynamic Bounds Work on Dense Rewards:}

Step-aware bounds improved CartPole by +17.9\%, validating the dynamic bounding approach for dense reward tasks even with PPO.

\textbf{3. Conflict with GAE:}
...
Hard clipping V(s') during bootstrapping may disrupt GAE's temporal smoothing, explaining why QBound hurts performance on sparse reward tasks where GAE excels.

\textbf{4. PPO Baseline Already Strong:}

PPO achieved 80\% success on LunarLander without QBound, compared to DQN's 11\%. This suggests:
\begin{itemize}
    \item PPO's advantage normalization and clipped objective provide natural stabilization
    \item Adding external bounds may over-constrain the value function
    \item QBound addresses instabilities that PPO already handles via different mechanisms
\end{itemize}

\subsubsection{Implications}

\textbf{QBound is fundamentally algorithm-dependent:}
\begin{enumerate}
    \item \textbf{Works for value-based methods (DQN):} Stabilizes Q-learning bootstrapping
    \item \textbf{Fails for continuous action actor-critic (DDPG/TD3):} Disrupts policy gradients
    \item \textbf{Conflicts with policy gradient methods (PPO):} Interferes with GAE
\end{enumerate}

\textbf{Recommended Application:} QBound is most effective for discrete action, value-based methods (DQN variants) where value function instability is the primary challenge and no built-in stabilization mechanisms exist.
...
\begin{itemize}
    \item \textbf{Discrete actions only:} QBound is fundamentally incompatible with continuous action spaces—hard clipping disrupts policy gradients (893\% degradation on Pendulum with DDPG/TD3)
    \item \textbf{Algorithm-dependent:} QBound works best for value-based methods (DQN variants). Extension to policy gradient methods (PPO) shows conflicts with Generalized Advantage Estimation, degrading LunarLander performance by -30.9\% despite PPO's strong 80\% baseline
    \item \textbf{Known bounds required:} QBound requires reasonably tight bounds derivable from environment structure
    \item \textbf{Environment-dependent:} Not universally beneficial; effectiveness varies by task characteristics
\end{itemize}
