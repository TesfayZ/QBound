...
\textbf{Overall success rate:} 40\% (6/15 algorithm-environment combinations show $>$10\% improvement), 13\% neutral, 47\% degradation. \textbf{Recommendations:} Use Hard QBound for discrete actions with positive dense rewards (CartPole-style survival tasks); use Soft QBound for continuous control (DDPG/TD3). \textit{Do not use} QBound for negative rewards (implicit upper bound satisfied), sparse rewards (insufficient signal), or on-policy methods (PPO). Results demonstrate QBound is a \textit{specialized technique} requiring careful environment analysis, not a universal improvement. Implementation imposes negligible computational overhead ($<$2\%). All experiments use 5 seeds with full reproducibility protocols.
...
For negative rewards (e.g., Pendulum: $r \in [-16, 0]$), the Bellman equation itself provides an implicit upper bound.
...
\begin{corollary}[Statistical Learning of Upper Bound]
Neural networks trained with TD learning on negative rewards learn $Q \leq 0$ through statistical regularization. Every gradient update minimizes $\mathcal{L} = (Q_\theta - Q_{\text{target}})^2$ where $Q_{\text{target}} \leq 0$. After 100,000+ updates with negative targets, the network learns to output $Q \leq 0$.
\end{corollary}

\textbf{Empirical verification:} Pendulum DQN ($r \in [-16,0]$) shows \textbf{0.0000 violations} of $Q > 0$ across 500 episodes with 5 seeds (Section~\ref{subsec:pendulum-results}), confirming the upper bound is implicitly satisfied. Explicit QBound becomes redundant, causing -3\% to -7\% performance degradation.

\subsubsection{Summary: When QBound Works}

\begin{table}[h]
\centering
\caption{QBound Effectiveness by Reward Sign (5 seeds)}
\label{tab:reward-sign-summary-theory}
\begin{tabular}{lccc}
\toprule
Reward Type & Natural Upper Bound? & QBound Benefit? & Empirical Result \\
\midrule
\textbf{Positive Dense} (CartPole) & No & \checkmark Yes & +12\% to +34\% \\
\textbf{Negative Dense} (Pendulum) & Yes ($Q \leq 0$) & $\times$ No & -3\% to -7\% \\
\textbf{Sparse Terminal} (GridWorld) & Yes & $\times$ No & $\approx 0\%$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key insight:} RL is reward maximization—the upper bound matters, the lower bound does not. For negative rewards, the upper bound is automatically satisfied via the Bellman equation, making QBound redundant.
...
\subsection{Part 6: Comprehensive Multi-Seed Evaluation}
...
\begin{enumerate}
    ...
    \item \textbf{Critical Comparative Analysis:} Direct comparison with Double DQN revealing environment-dependent behavior of generic pessimism—Double DQN fails catastrophically on dense-reward tasks (-66\% on CartPole) while QBound's environment-aware bounds perform well when reward sign is appropriate
...
\textbf{Critical finding on reward sign dependence (5 seeds, 50 experiments):}
\begin{itemize}
    ...
    \item \textbf{Continuous control (DDPG/TD3):} +15-25\% improvement with Soft QBound. DDPG achieves +25.0\% (baseline: -213.10 $\pm$ 89.26, QBound: -159.79 $\pm$ 11.66) with 87\% variance reduction. TD3 achieves +15.3\% with 51\% variance reduction. \textbf{Key mechanism:} Soft QBound ($Q_{\text{soft}} = Q_{\max} - \text{softplus}(Q_{\max} - Q)$) provides \textit{stabilization} rather than strict bounding, preserving gradients for continuous control.

    \item \textbf{On-policy (PPO):} -20.4\% degradation (baseline: -784.96 $\pm$ 269.14, QBound: -945.09 $\pm$ 116.08). \textbf{Explanation:} PPO suffers less from overestimation because: (1) value function updated with recent on-policy samples, not bootstrapped from arbitrary past experiences; (2) policy uses advantage estimates $A(s,a) = Q(s,a) - V(s)$, relative comparisons less sensitive to absolute errors; (3) PPO includes value clipping, providing implicit stabilization. QBound interferes with carefully tuned policy-value interaction.
...
\midrule
\multicolumn{4}{@{}l}{\textit{\textbf{Policy Gradient (On-Policy):}}} \\
PPO & Any & \textbf{Avoid: on-policy reduces overestimation} & Pendulum: -20.4\% \\
\bottomrule
\end{tabular}
\end{table}
