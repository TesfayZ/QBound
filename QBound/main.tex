...
\bottomrule
\end{tabular}
\caption{PPO value bounding configurations. Both use Soft QBound with quadratic penalty $\mathcal{L} = \max(0, V-V_{\max})^2 + \max(0, V_{\min}-V)^2$ and penalty weight $\lambda = 0.1$. Static bounds used because PPO has different training dynamics than Q-learning.}
