...
For negative reward environments (Pendulum: $r \in [-16, 0]$), we tested both hard clipping QBound (algorithmic \texttt{torch.clamp}) and architectural QBound (output activation: $Q = -\text{softplus}(\text{logits})$) across multiple algorithms. Both approaches degrade performance for most algorithms: hard clipping degrades DQN by 6.8\% and DDQN by 2.3\%; architectural QBound degrades DQN by 3.3\%, DDPG by 8.0\%, and PPO by 10.8\%. TD3 is an exception, showing 4.1\% improvement with architectural QBound ($-183.25 \rightarrow -175.66$) but with doubled variance ($\pm 40.15$ vs $\pm 23.36$), suggesting a unique interaction with TD3's twin critic architecture.
...
\begin{itemize}
    ...
    \item \textbf{Empirical result:} Pendulum DQN shows \textbf{-0.5\% degradation with 160\% variance increase} (5 seeds)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Empirical result:} Pendulum shows \textbf{+2.5\% (DQN) to +7.2\% (TD3) improvement} with reduced variance (5 seeds)
\end{itemize}

\textbf{Key insight:} Architectural constraints \textit{guide exploration} within the correct range from initialization, while hard clipping \textit{corrects exploration} after violations. For positive rewards (CartPole), hard clipping succeeds because it aligns with natural network growth—preventing unbounded increase without opposing initialization bias. For negative rewards, architectural constraints succeed by eliminating exploration-correction conflicts entirely.
...
\textbf{Negative Dense} (Pendulum DQN) & Yes ($Q \leq 0$) & -0.5\% & +2.5\% & Architectural \\
\textbf{Negative Dense} (Pendulum DDPG/TD3) & Yes ($Q \leq 0$) & N/A & +4.8\% to +7.2\% & Architectural \\
\textbf{Sparse Terminal} (GridWorld) & Yes & $\approx 0\%$ & N/A & Neither \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key insight:} QBound is a \textit{principle} (bound Q-values to environment structure) with multiple \textit{implementations}. For positive rewards, hard clipping works because it prevents unbounded growth. For negative rewards, architectural constraints work because they align with gradient flow while hard clipping creates conflicts. Implementation method must match reward structure.
...
\begin{table}[h]
\centering
\caption{Pendulum DQN: Hard Clipping vs Architectural QBound (5 seeds)}
...
DQN + Hard Clipping & -162.25 & 14.48 & \textcolor{red}{-0.5\%} & \textcolor{red}{+160\%} \\
DQN + Architectural & \textbf{-148.38} & 9.22 & \textcolor{green}{+2.5\%} & \textcolor{green}{-27\%} \\
\midrule
Double DQN Baseline & -157.45 & 10.11 & --- & --- \\
Double DQN + Hard Clipping & -171.11 & 13.89 & \textcolor{red}{-8.7\%} & \textcolor{red}{+37\%} \\
Double DQN + Architectural & \textbf{-155.21} & 8.45 & \textcolor{green}{+1.4\%} & \textcolor{green}{-16\%} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key Finding:} Hard clipping degrades performance and increases variance due to gradient blocking. Architectural QBound ($Q = -\text{softplus}(\text{logits})$) improves both performance and variance by:
\begin{itemize}
    \item \textbf{Smooth gradients:} $\frac{\partial Q}{\partial \text{logits}} = -\text{sigmoid}(\text{logits})$ is never zero
    \item \textbf{Natural constraints:} Network outputs stay in $(-\infty, 0]$ by construction (zero violations)
    \item \textbf{No conflicts:} Learning proceeds without fighting against clipping boundaries
\end{itemize}

\textbf{Interpretation:} The Bellman equation naturally constrains $Q \leq 0$ for negative rewards, but architectural enforcement aligns with gradient flow while hard clipping creates conflicts. Implementation method matters more than theoretical correctness.

\paragraph{Pendulum DDPG/TD3 (Continuous Actions): Architectural QBound Succeeds}

\textbf{Configuration:} Continuous actions, Architectural QBound ($Q = -\text{softplus}(\text{logits})$), $Q_{\min} = -1800$, $Q_{\max} = 0$
...
DDPG + Architectural & \textbf{-173.54} & 43.08 & \textcolor{green}{+4.8\%} & -37\% \\
TD3 Baseline & -171.93 & 39.68 & --- & --- \\
TD3 + Architectural & \textbf{-159.51} & 28.11 & \textcolor{green}{+7.2\%} & +29\% \\
PPO Baseline & -695.03 & 354.22 & --- & --- \\
PPO + Architectural & -817.18 & 170.41 & \textcolor{red}{-17.6\%} & +52\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Why Architectural QBound Works for Actor-Critic:}
\begin{itemize}
    \item \textbf{Smooth critic gradients:} Essential for policy gradient methods, preserved by softplus activation
    \item \textbf{Natural stabilization:} Network learns within correct range without explicit clipping conflicts
    \item \textbf{Variance reduction:} DDPG and TD3 show dramatic variance reduction (though PPO does not benefit)
\end{itemize}

\textbf{PPO Failure:} On-policy methods have natural overestimation reduction and built-in value clipping. Architectural constraints interfere rather than help.

\textbf{Key Insight:} Architectural QBound ($Q = -\text{softplus}(\text{logits})$) successfully extends QBound to negative rewards for off-policy actor-critic methods by aligning with gradient flow requirements.
...
\end{table}
 ...
 QBound is \textit{not} universally effective. Success depends critically on reward sign and density:
\begin{itemize}
