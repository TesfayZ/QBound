...
\begin{table}[h]
\centering
\caption{QBound Success Rate Summary (14 Algorithm-Environment Combinations)}
\label{tab:overall-success}
\begin{tabular}{lcccc}
\toprule
Category & Combinations & Success ($>$5\%) & Neutral ($\pm$5\%) & Failure ($<$-5\%) \\
\midrule
Positive Dense Rewards & 4 & 4 (100\%) & 0 & 0 \\
Negative Dense (TD3) & 1 & 1 (100\%) & 0 & 0 \\
Negative Dense (Others) & 3 & 0 & 1 & 2 (67\%) \\
Sparse Terminal Rewards & 2 & 0 & 2 (100\%) & 0 \\
State-Dependent Negative & 4 & 0 & 2 & 2 (50\%) \\
\midrule
\textbf{Overall} & \textbf{14} & \textbf{5 (36\%)} & \textbf{5 (36\%)} & \textbf{4 (28\%)} \\
\bottomrule
\end{tabular}
\end{table}

 QBound is \textit{not} universally effective. Success depends critically on reward sign and algorithm:
\begin{itemize}
    \item \checkmark \textbf{Strong success (36\%):} Positive dense rewards (CartPole: 12-34\% across all variants), TD3 on negative rewards (4.1\%)
    \item \textcolor{gray}{$\sim$} \textbf{Neutral (36\%):} Sparse terminal rewards (GridWorld, FrozenLake), DQN on negative rewards (-3.3\%)
    \item $\times$ \textbf{Failure (28\%):} DDPG (-8.0\%), PPO (-10.8\%), MountainCar/Acrobot DDQN (-47\%, -3.6\%)
\end{itemize}
