#!/usr/bin/env python3
"""
Comprehensive analysis of LunarLander 4-way comparison experiment
"""
...
def analyze_agent(name, data):
    ...
    # Success rate (reward > 200 is considered success for LunarLander)
    stats['success_rate'] = np.sum(rewards > 200) / len(rewards) * 100
    stats['final_100_success_rate'] = np.sum(rewards[-100:] > 200) / 100 * 100

    return stats

def main():
    # Find the latest LunarLander results
    results_dir = Path('/root/projects/QBound/results/lunarlander')
    result_files = sorted(results_dir.glob('4way_comparison_*.json'))

    if not result_files:
        print("No LunarLander results found!")
        return
    ...
    agents = ['dqn', 'qbound_static_dqn', 'ddqn', 'qbound_static_ddqn']
    agent_names = {
        'dqn': 'Baseline DQN',
    ...
    print(f"\n{'Agent':<20} {'Mean±Std':<20} {'Max':<10} {'Final 100':<20} {'Success %':<12} {'Final Success %':<15}")
    print("-" * 115)
        ...
        for agent in agents:
            if agent in results['evaluation']:
                ...
                eval_stats.append({
                    ...
                    'std': np.std(rewards),
                    'max': np.max(rewards),
                    'success_rate': np.sum(rewards > 200) / len(rewards) * 100
                })

        print(f"\n{'Agent':<20} {'Mean±Std':<20} {'Max':<10} {'Success %':<12}")
        print("-" * 70)
    ...
    colors = {'dqn': 'blue', 'qbound_static_dqn': 'green',
              'ddqn': 'purple', 'qbound_static_ddqn': 'red'}
    ...
    for idx, agent in enumerate(agents):
        if agent in results['training']:
            ...
            # Success threshold
            ax.axhline(y=200, color='black', linestyle='--', alpha=0.5, label='Success (200)')

            ax.set_xlabel('Episode', fontsize=12)
            ax.set_ylabel('Total Reward', fontsize=12)
            ax.set_title(f'{agent_names[agent]}', fontsize=14, fontweight='bold')
            ax.grid(True, alpha=0.3)
            ax.legend(fontsize=10)

    plt.tight_layout()
    plot_file = plots_dir / f'lunarlander_learning_curves_{results["timestamp"]}.png'
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
    print(f"✓ Saved learning curves: {plot_file.name}")

    # PDF version for paper
    plot_file_pdf = plots_dir / f'lunarlander_learning_curves_{results["timestamp"]}.pdf'
    ...
    ax.set_title('LunarLander-v3: QBound Comparison', fontsize=16, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=12)

    plt.tight_layout()
    plot_file = plots_dir / f'lunarlander_comparison_{results["timestamp"]}.png'
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
    print(f"✓ Saved comparison plot: {plot_file.name}")

    plot_file_pdf = plots_dir / f'lunarlander_comparison_{results["timestamp"]}.pdf'
    plt.savefig(plot_file_pdf, bbox_inches='tight')
    print(f"✓ Saved PDF version: {plot_file_pdf.name}")
    plt.close()
    ...
    ax.axhline(y=200, color='red', linestyle='--', linewidth=2, label='Success Threshold (200)')
    ax.set_xlabel('Agent', fontsize=14)
    ax.set_ylabel('Mean Reward (Final 100 Episodes)', fontsize=14)
    ax.set_title('LunarLander-v3: Final Performance Comparison', fontsize=16, fontweight='bold')
    ax.set_xticks(x_pos)
    ...
    plt.tight_layout()
    plot_file = plots_dir / f'lunarlander_bar_comparison_{results["timestamp"]}.png'
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
    print(f"✓ Saved bar chart: {plot_file.name}")

    plot_file_pdf = plots_dir / f'lunarlander_bar_comparison_{results["timestamp"]}.pdf'
    plt.savefig(plot_file_pdf, bbox_inches='tight')
    print(f"✓ Saved PDF version: {plot_file_pdf.name}")
    plt.close()
